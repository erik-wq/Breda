...
namespace Tmpl8 {
	...
	void vec2::rotateVect2(vec2* vector, float angle)
	{
		float s = std::sin(angle);
		float c = std::cos(angle);
		float newX = vector->x * c - vector->y * s;
		float newY = vector->x * s + vector->y * c;
		vector->x = newX;
		vector->y = newY;
	}
		...
		while (!exitapp)
		{
			...
			while (SDL_PollEvent(&event))
			{
				...
				case SDL_MOUSEMOTION:
					game->MousePositionUpdate(event.motion.x, event.motion.y); // tracking mouse position
